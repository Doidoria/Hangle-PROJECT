...
const Setting = () => {
  ...
  const handleUsernameChange = async () => {
    ...
    if (!newUsername || newUsername === currentUsername) {
      Swal.fire({ icon: 'info', title: '변경할 이름을 입력해주세요.',
        confirmButtonColor: '#10B981',
      });
      return;
    }
    ...
    try {
      ...
      Swal.fire({ icon: 'success', title: '이름 변경 완료', text: '이름이 성공적으로 변경되었습니다.',
        confirmButtonColor: '#10B981',
      });
      setNewUsername('');
      setIsEditingUsername(false);
    } catch (error) {
      Swal.fire({ icon: 'error', title: '이름 변경 실패', text: error.response?.data?.error || '서버 오류가 발생했습니다.',
  ...
  const handleEmailChange = async () => {
    if (!isVerified) {
      Swal.fire({ icon: 'warning', title: '본인 인증 필요', text: '본인 인증 후 회원정보를 변경할 수 있습니다.',
        confirmButtonColor: '#10B981',
      });
      return;
    }
    ...
    if (!newEmail || newEmail === currentEmail) {
      Swal.fire({ icon: 'info', title: '변경할 이메일을 입력해주세요.',
        confirmButtonColor: '#10B981',
      });
      return;
    }
    ...
    try {
      ...
      if (resp.data.message?.includes('로그아웃')) {
        Swal.fire({ icon: 'success', title: '이메일 변경 완료',
      ...
      // 단순 이메일만 수정(로그아웃 아님)
        ...
        Swal.fire({ icon: 'success', title: '이메일 변경 완료', text: '이메일이 성공적으로 변경되었습니다.',
          confirmButtonColor: '#10B981',
        });
      ...
      Swal.fire({ icon: 'error', title: '이메일 변경 실패',
  ...
  const handleDeleteAccount = async () => {
    if (!isVerified) {
      Swal.fire({ icon: 'warning', title: '본인 인증 필요',
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({ icon: 'success', title: '계정 삭제 완료', text: '계정이 정상적으로 삭제되었습니다.',
          confirmButtonColor: '#10B981',
        }).then(() => {
          localStorage.clear();
        ...
        Swal.fire({ icon: 'error', title: '삭제 실패', text: '계정을 삭제하는 중 문제가 발생했습니다.',
          confirmButtonColor: '#d33',
        });
      }
    }
            ...
            <div className="settings-content">
              ...
              <div className='setting-card'>
                <div className="verification-section">
                  ...
                  {isVerified && (
                      <button 
                          className="action-button verify-button verified" 
                          disabled>
                          인증 완료됨
                      </button>
                  )}
