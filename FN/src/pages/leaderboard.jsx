...
const Leaderboard = () => {
    ...
    const [originalLeaderboard, setOriginalLeaderboard] = useState([]);
    ...
    const size = Number(searchParams.get('size') || 4); //size 변수 추가
    ...
    useEffect(() => {
        const fetchCompetitions = async () => {
            try {
                ...
                setOriginalLeaderboard(originalList);
    ...
    useEffect(() => {
        ...
        const filtered = originalLeaderboard.filter(item =>
            ...
            <section className="section-wrap">
                <div className="leaderboard-section">
                    <div className="leaderboardHeader">
                        ...
                        <img src="/image/comp1img.svg" alt="leaderboardImg" />

                    </div>

                    <form className="leaderboardsearch" onSubmit={onSearch}>
                        <input name="keyword" placeholder="검색어" />
                        <button className="btn3" type="submit">검색</button>
                    </form>
                    ...
                    <div>
                        ...
                        {data.content.map(({ comp, entries }) => (
                            <div className="leaderboard" key={comp.id}>
                                <h3>{comp.title}</h3>
                                <div className="leaderboardcard" style={{ overflowX: "auto" }}>
                    ...
                    <div className="pagebtn4">
                        <button className="btn6" disabled={page <= 0} onClick={() => movePage(page - 1)}>이전</button>
                        <span>페이지 {page + 1} / {Math.max(data.totalPages, 1)}</span>
                        <button className="btn6" disabled={page + 1 >= data.totalPages} onClick={() => movePage(page + 1)}>다음</button>
                    </div>
...
//                 .collect(Collectors.groupingBy(LeaderboardEntryDto::getCompetitionId)) // 대회별 그룹
//                 .values().stream()
//                 .map(list -> list.stream().limit(5).toList()) // 각 대회 5개씩
//                 .flatMap(List::stream) // 다시 하나의 리스트로 합치기
//                 .toList();